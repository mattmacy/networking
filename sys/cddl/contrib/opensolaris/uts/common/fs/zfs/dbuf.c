...
dbuf_read_impl(dmu_buf_impl_t *db, zio_t *zio, uint32_t *flags)
{
	...
	if (db->db_blkptr == NULL || BP_IS_HOLE(db->db_blkptr) ||
		...
		dbuf_set_data(db, arc_alloc_buf(dn->dn_objset->os_spa,
		    db, type, db->db.db_size));
		DB_DNODE_EXIT(db);
...
dbuf_new_dirty_record_accounting(dbuf_dirty_state_t *dds)
{
	...
	if (db->db_blkid != DMU_BONUS_BLKID) {
		...
		if (db->db_blkid != DMU_SPILL_BLKID)
			dds->dds_do_free_accounting = dbuf_block_freeable(db);
	}
}
...
dbuf_dirty_exit(dbuf_dirty_state_t *dds)
{
	...
	if (!dds->dds_txg_already_dirty) {
		if (dds->dds_do_free_accounting) {
			...
			dmu_objset_willuse_space(dn->dn_objset, -willfree, dds->dds_tx);
		}
...
dbuf_destroy(dmu_buf_impl_t *db)
{
	...
	if (db->db_is_ephemeral && db->db_blkptr != NULL)
		bp = *db->db_blkptr;
...
top:
	...
	if (dbs != NULL && (dbs->dmu_ctx->flags & DMU_CTX_FLAG_READ)) {
		if (db->db_state == DB_CACHED) {
			/* Dbuf is already at the desired state. */
			dmu_buf_set_rele(dbs, /*err*/0);
		} else
			dmu_context_node_add(&db->db_dmu_buf_sets, dbs);
	}
	mutex_exit(&db->db_mtx);
